...
ggplot(my_ny_noaa, aes(x = tmax, y = tmin)) + 
  geom_hex() # tmax vs tmin bin plot
```
...
``` r
ggplot(dis_snow, aes(x = factor(year), y = snow, fill = year)) +
  geom_boxplot()
```

<img src="p8105_hw3_yx2640_files/figure-gfm/unnamed-chunk-12-2.png" width="90%" />
